...
if (-not [string]::IsNullOrWhiteSpace($SwitchHeaderReferenceFolder)) {
    ...
    $exact = Join-Path $SwitchHeaderReferenceFolder 'achievement_controller.nsc'
...
Copy-Item -LiteralPath $Source -Destination (Join-Path $OutputDirectory 'achievement_controller.sc') -Force
...
$ragemenuSnapshot = Join-Path $PSScriptRoot '..\source\ragemenu.sc'
if (Test-Path -LiteralPath $ragemenuSnapshot) {
    Copy-Item -LiteralPath $ragemenuSnapshot -Destination (Join-Path $OutputDirectory 'ragemenu.sc') -Force
}
$sco = Join-Path $OutputDirectory 'achievement_controller.sco'
$pcNsc = Join-Path $OutputDirectory 'achievement_controller.pc.nsc'
$switchNsc = Join-Path $OutputDirectory 'achievement_controller.nsc'
